...
public class ImageServiceImpl implements ImageService {
	...
	@Override
	public Image uplaodImageSon(MultipartFile file, Long idSon) throws IOException {
		Song s = new Song();
		s.setIdSong(idSon);
		return imageRepository.save(Image.builder().name(file.getOriginalFilename()).type(file.getContentType())
				.image(file.getBytes()).song(s).build());
	}

	@Override
	public List<Image> getImagesParSon(Long sonId) {
		Song s = songRepository.findById(sonId).get();
		return s.getImages();
	}
}